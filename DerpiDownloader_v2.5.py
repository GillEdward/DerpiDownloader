...
def DownloadPage(page):
    ...
    for image in data:
        ...
        if fileCounter == len(image['images']):
            print('Page' + str(page) + " Clear\n")
        else:
            print('Page' + str(page) + " Unclear, rechecking\n")
...
key = input("输入你的API-KEY(不要告诉别人), 你可以在账户里找到你的API-KEY\n\thttps://trixiebooru.org/registrations/edit or \n\thttps://derpibooru.org/registrations/edit\n:")
artist = input('artist: ')
tags = input('tags: ')
root = 'imageDownload'
filePath = root + '/' + artist
if len(tags) != 0:
    filePath += '+' + tags
